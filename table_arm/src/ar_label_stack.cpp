     ...
     if(ar_num77_ready== 0 )  
       {
         ...
         if(ar_num77_count>30) //前30帧的数据不使用
         {
           ar_num77_ready=2;
     ...
     if(ar_num88_ready== 0 )  
       {
         ...
         if(ar_num88_count>30) //前30帧的数据不使用
         {
           ar_num88_ready=2;
     ...
     if(ar_num99_ready== 0 )  
       {
         ...
         if(ar_num99_count>30) //前30帧的数据不使用
         {
           ar_num99_ready=2;
    ...
    while(ros::ok())
   {
      ...
      if( arm_state=="ready" )
      {
       ...
       // while(!arm_success)
        ...
        arm_put(ar_sequence); //根据ar标签的号码将标签块放置到对应位
      } 
    ros::spinOnce();
  }
  ros::shutdown(); 
...
void arm_put(int put_sequence)
{
    ...
    //根据r标签的号码判断放置位置
    if (put_sequence == 1)        {arm.setNamedTarget("ar_stack_put_position1");  arm.move();  sleep(1); }
    else if (put_sequence == 2)   {arm.setNamedTarget("ar_stack_put_position2");    arm.move();  sleep(1); }
    else if (put_sequence == 3)   {arm.setNamedTarget("ar_stack_put_position3");  arm.move();  sleep(1); ROS_INFO("stack ar_label is done");}
